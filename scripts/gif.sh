...
write_zoom_frames() {
  ...
  while [ "$i" -lt "$frames" ]; do
    ...
    magick "$base" -filter LanczosSharp -resize "${scale}%" \
      ...
      -define png:compression-level=9 "$frames_dir/frame_$(printf '%04d' "$i").png"
    i=$((i + 1))
  done
  rm -f "$base"
}
...
make_gif() {
  ...
  first_frame=$(find "$frames_dir" -maxdepth 1 -type f -name '*.png' | sort | head -n 1)
  [ -n "$first_frame" ] || {
